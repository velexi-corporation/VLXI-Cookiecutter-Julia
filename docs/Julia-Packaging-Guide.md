...
* (OPTIONAL) Enable creation of version-specific documentation.
  ...
  * Make sure that the `docs` GitHub Action job builds and deploys documentation using a
    `make.jl` file containing a call to `deploydocs()` with the `versions` keyword set
    to an appropriate value (the default value is reasonable).

    Replace the `run` command in the default `CI.yml` generated by `PkgTemplates` with the
    following:
...
* Publish a GitHub release associated with the package release.
  ...
  * __Recommendation__. If the `draft` parameter has been set to `true` in `TagBot.yml`, a
    ...
    to the
    the release notes, if needed, after the GitHub release has been published.
